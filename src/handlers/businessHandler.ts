...
export const sendTemplateHandler = async (c: Context) => {
  try {
...
export const createWhatsappTemplateHandler = async (c: Context) => {
  const body = await c.req.json();
...
//get the templates
export const getWhatsappTemplatesHandler = async (c: Context) => {
  try {
...
export const getWhatsappTemplateByNameHandler = async (c: Context) => {
  try {
    ...
    console.error(error.response?.data || error.message);
    return c.json(
      { error: error.response?.data || error.message || "Error fetching template" },
      500
    );
  }
};
